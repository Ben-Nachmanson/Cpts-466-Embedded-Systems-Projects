...
//#define SYSCTL_RCGC1_UART1      0x00000001  // UART1 Clock Gating Control
...
void UART_Init(void){
  ...
  UART1_IBRD_R = 325;                    // IBRD = int(50,000,000 / (16 * 9600)) = int(27.1267)
...
//------------UART_OutChar------------
// Output 8-bit to serial portit ASCII character to be transferred
// Output: none
// Input: letter is an 8-b
...
// Output: none
// Variable format 1-10 digitsr with no space before or after
...
void UART_ConvertedDistance(unsigned long n){
// This function uses recursion to convert decimal number
//   of unspecified length as an ASCII string
	char str[7] = "0000000";
	char outputStr[9]= "0.000 cm";

	int fix = 0;

  fix = sprintf(str, "%lu", n);
	...
	{
		if(fix - 1 >= 0) outputStr[4] = str[fix-1];
		if(fix - 2 >= 0) outputStr[3] = str[fix-2];
		if(fix - 3 >= 0) outputStr[2] = str[fix-3];
		if(fix - 4 >= 0) outputStr[0] = str[fix-4];
