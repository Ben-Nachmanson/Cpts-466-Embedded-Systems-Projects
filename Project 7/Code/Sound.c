...
const unsigned char SineWave[16] = {8,10,12,14,14,14,12,10,8,6,4,2,1,2,4,6};
...
void DAC_Init(void){unsigned long volatile delay;
  ...
  GPIO_PORTB_AMSEL_R &= ~0x3C;      // no analog 
  GPIO_PORTB_PCTL_R &= ~0x00FFFF00; // regular function
  GPIO_PORTB_DIR_R |= 0x3C;      // make PB2-0 out
  GPIO_PORTB_AFSEL_R &= ~0x3C;   // disable alt funct on PB2-0
  GPIO_PORTB_DEN_R |= 0x3C;      // enable digital I/O on PB2-0
}
